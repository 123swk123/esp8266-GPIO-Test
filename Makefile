...
CFLAGS = -Os -g -std=gnu90 -Wpointer-arith -Wundef -Werror -Wl,-EL -fno-inline-functions -nostdlib -mlongcalls -mtext-section-literals -mno-serialize-volatile -D__ets__ -DICACHE_FLASH -DUSE_US_TIMER
...
.PHONY: all checkdirs clean flash flashinit flashonefile rebuild flashread
...
flashwifi:
	$(vecho) "Flashing wifi bin @ 0x3FE000"
	$(ESPTOOL) -p $(ESPPORT) write_flash 0x3FE000 $(FW_BASE)/0x3FE000.bin
	
flashread:
	$(vecho) "Reading blank.bin wifi user settings @ 0x3FE000"
	$(ESPTOOL) -p $(ESPPORT) -b $(BAUD) read_flash 0x3FE000 4096 $(FW_BASE)/0x3FE000.bin
